...
public class ProductServiceImpl implements ProductService {
	...
	public ProductDTO addProduct(ProductDTO productDTO) throws BusinessException {
		...
		for (ProductDTO dto : productList) {
			...
			// && dto.getProductId().equals(productDTO.getProductId())
				...
				be.setErrocode("Dup_001");
				be.setErrorMsg("Product Allready Exist");
				throw be;
			}
		}
	...
	public ProductDTO getProduct(Long productId) {
		for (ProductDTO dto : productList) {
         ...
         return null;
		// return DataSerializer.deserailizeProduct(productId);
	}
	...
	public ProductDTO updateProductPriceNonDB(Long productId, Double newPrice) {
		...
		if (null != dto) {
			...
			// find the product with matching Id and replace it from list
	...
	public ProductDTO deleteProductByIdNonDB(Long productId) {
		 ...
		 * null;
		 * 
		 * while(productDtoItr.hasNext()){ dto = productDtoItr.next();
		 * if(dto.getProductId().equals(productId)) { productDtoItr.remove(); }
		 * 
		 * } return dto;
		 */
